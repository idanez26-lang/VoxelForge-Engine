...
Asset::Voxel::VoxelDocument MakeDocument()
{
    ...
    const auto loaded =
        Asset::Voxel::VoxDocumentLoader{}.Build(source, "rotation-smoke.vox");
    Require(loaded.Succeeded() && loaded.Document,
        "Rotation smoke document must build.");
    ...
    const Asset::Voxel::VoxelDocument& document)
{
         ...
         index < document.GetPalette().size(); ++index)
    {
        ...
        Require(model.Palette().Set(index,
            ...
            "Unable to initialize rotation smoke palette.");
    }
    const auto* source = document.GetModel(0U);
    Require(source != nullptr, "Rotation smoke document has no model.");
    Voxel::VoxelGrid grid;
    const auto dimensions = source->Dimensions();
    Require(grid.Resize(dimensions.X, dimensions.Y, dimensions.Z),
        "Unable to initialize rotation smoke grid.");
...
public:
    ...
    {
        ++rebuilds_;
        const auto synchronized = meshCache_.Synchronize(*document_, 15U);
...
VoxelStamp MakeStamp()
{
    ...
    const auto stamp = VoxelStamp::TryCreate(
        {Core::UUID{0x5354414d503135ULL}, "stamp-15-smoke"},
        {{0, 0, 0}, {1, 0, 1}, {2U, 1U, 2U}},
...
void RunScenario()
{
    ...
    Require(placement.Begin(
                MakeStamp(), document, 15U, 0U,
                {5 * StampFixedPoint::UnitsPerVoxel,
                 StampFixedPoint::UnitsPerVoxel,
                 5 * StampFixedPoint::UnitsPerVoxel}).Succeeded,
        "Preview must begin.");
    Require(placement.RotateClockwise(document, 15U).Succeeded &&
                placement.RotateClockwise(document, 15U).Succeeded &&
                placement.QuarterRotation() == 2U &&
                placement.CurrentPreview() != nullptr &&
                placement.CurrentPreview()->Transform.QuarterTurns == 2U,
    ...
    for (const StampPlannedVoxel& voxel : plannedVoxels)
    {
        Require(document.GetVoxel(voxel.WorldPosition).has_value(),
            "Placed document cells must exactly equal the rotated plan.");
    }

    Require(static_cast<bool>(history.Undo(editSession)),
        "Rotated placement Undo must succeed.");
    for (const StampPlannedVoxel& voxel : plannedVoxels)
    {
        Require(!document.GetVoxel(voxel.WorldPosition).has_value(),
            "Undo must remove the exact rotated placement.");
    }
    Require(static_cast<bool>(history.Redo(editSession)),
        "Rotated placement Redo must succeed.");
    for (const StampPlannedVoxel& voxel : plannedVoxels)
    {
        Require(document.GetVoxel(voxel.WorldPosition).has_value(),
            "Redo must restore the exact rotated placement.");
    }
    Require(placement.Rebuild(document, 15U).Succeeded &&
                placement.CurrentPreview() != nullptr &&
                placement.CurrentPreview()->State ==
                    VoxelPreviewState::Overlap,
        "Preview must persist after placement and Redo.");
    Require(placement.Cancel() && !placement.IsActive(),
        "Escape/Clear must end rotation placement without mutation.");
}
